...
void Application::CreateFramebuffers()
{
  ...
  for (size_t i = 0; i < swap_chain_image_views_.size(); ++i) {
    std::array<vk::ImageView, 2> attachments = {swap_chain_image_views_[i],
                                                depth_image_view_};
...
void Application::CreateCommandBuffers()
{
  ...
  for (size_t i = 0; i < command_buffers_.size(); ++i) {
    ...
    command_buffers_[i].bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                                           pipeline_layout_, 0,
                                           descriptor_sets_[i], {});
    command_buffers_[i].drawIndexed(static_cast<uint32_t>(indices.size()), 1, 0,
                                    0, 0);
...
void Application::DrawFrame()
{
  ...
  auto wait_for_fence_result =
      logical_device_.waitForFences(in_flight_fences_[current_frame_], VK_TRUE,
  ...
  if (images_in_flight_[image_index]) {
    auto wait_for_image_fence =
        logical_device_.waitForFences(images_in_flight_[image_index], VK_TRUE,
                                      std::numeric_limits<uint64_t>::max());
...
void Application::UpdateUniformBuffer(uint32_t index)
{
  ...
  void* data =
      logical_device_.mapMemory(uniform_buffers_memory_[index], 0, sizeof(ubo));
...
void Application::CreateDescriptorSets()
{
  ...
  for (size_t i = 0; i < swap_chain_images_.size(); ++i) {
    ...
    vk::DescriptorImageInfo image_info(texture_sampler_, texture_image_view_,
    ...
    vk::ImageUsageFlags usage, vk::MemoryPropertyFlags properties)
{
  ...
  vk::ImageCreateInfo image_info(
      vk::ImageCreateFlags(), vk::ImageType::e2D, format, {width, height, 1}, 1,
...
void Application::CreateTextureImage()
{
  ...
  std::tie(texture_image_, texture_image_memory_) = CreateImage(
      texture_width, texture_height, vk::Format::eR8G8B8A8Srgb,
...
void Application::CreateDepthResources()
{
  ...
  TransitionImageLayout(depth_image_, depth_format, vk::ImageLayout::eUndefined,
                        vk::ImageLayout::eDepthStencilAttachmentOptimal);
}
